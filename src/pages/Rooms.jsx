...
const Rooms = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${BACKEND_API_URL}/hotels/${id}/`
        );
        const data = await response.json();
        if (data) {
          setHotelData(data);
          // Set rooms data from the rooms array in the response
  ...
  // Calculate total amount and number of nights when dates or rooms change
  useEffect(() => {
    if (checkinDate && checkoutDate && selectedRoom) {
      ...
      const amount = parseFloat(selectedRoom.charge_per_night) * roomsBooked * nights;
      setTotalAmount(amount);
    } else {
      setTotalAmount(0);
      setNumberOfNights(0);
    }
  }, [checkinDate, checkoutDate, roomsBooked, selectedRoom]);
        ...
        <div className="relative bg-gradient-to-r from-purple-600 to-indigo-800 py-20 text-white px-6">
          ...
          <div className="container mx-auto px-4 relative z-10 text-center">
            <h1 className="text-4xl md:text-5xl font-bold mb-4">
              {hotelData?.name || "Our Rooms"}
            </h1>
            <p className="text-xl md:text-2xl max-w-2xl mx-auto">
              {hotelData?.location ? `Located in ${hotelData.location}` : "Experience luxury and comfort in our beautifully designed rooms"}
            </p>
          </div>
        </div>
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex justify-between items-center mb-4">
                        <div className="flex items-center">
                          ...
                          <span className="text-gray-700">Occupancy: {room.occupancy_capacity}</span>
                        </div>
                        <div className="flex items-center">
                          <FaRulerCombined className="text-purple-600 mr-2" />
                          <span className="text-gray-700">{room.smoking_room ? "Smoking" : "Non-smoking"}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <div className="text-right">
                          <p className="text-2xl font-bold text-purple-600">
                            KES {parseFloat(room.charge_per_night).toLocaleString()}
                          </p>
                ...
                {(checkinDate && checkoutDate) && (
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <div className="flex justify-between mb-2">
                      ...
                      <span className="font-medium">KES {parseFloat(selectedRoom.charge_per_night).toLocaleString()} per night</span>
                    </div>
                    <div className="flex justify-between mb-2">
